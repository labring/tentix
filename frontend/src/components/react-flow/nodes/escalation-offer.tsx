...
const EscalationOffer: React.FC<NodeProps<Node<EscalationOfferNodeData>>> = ({
  ...
  const updateCondition = useCallback(
    (index: number, value: string) => {
      setConditions((prev) => prev.map((cond, i) => (i === index ? value : cond)));
      const targetHandleId = conditionHandles[index]?.id;
      if (!targetHandleId) return;
  ...
  const getNormalSourceTopPx = useCallback((i: number) => {
    // 复用旧的百分比方案，但换算为像素并固定锚定在 BaseNode 高度
  ...
  return (
    <div className="relative group">
      <BaseNode className="w-[300px] bg-white border border-slate-200 shadow-lg hover:shadow-xl transition-all duration-200 rounded-lg overflow-hidden flex flex-col" style={{ height: `${BASE_NODE_HEIGHT}px` }}>
