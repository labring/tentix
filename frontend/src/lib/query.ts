...
export const analyticsOverviewQueryOptions = (filterParams?: {
  startDate?: string;
  endDate?: string;
  agentId?: string;
  isToday?: boolean;
  ...
  queryOptions({
    queryKey: ["analyticsOverview", filterParams],
    queryFn: async () => {
      const searchParams = buildAnalyticsParams(filterParams);
      
      const data = await apiClient.analytics.overview
...
export const hotIssuesQueryOptions = (filterParams?: {
  ...
  queryOptions({
    ...
    queryFn: async () => {
      ...
      const data = await apiClient.analytics["hot-issues"]
        .$get({ query: Object.fromEntries(searchParams) })
        .then((r) => r.json());
