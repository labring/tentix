...
CREATE TYPE "tentix"."ticket_history_type" AS ENUM('create', 'update', 'assign', 'upgrade', 'transfer', 'makeRequest', 'resolve', 'close', 'other');--> statement-breakpoint
...
CREATE TABLE "tentix"."ticket_session_members" (
	...
	"last_viewed_at" timestamp(3) DEFAULT now() NOT NULL,
...
ALTER TABLE "tentix"."requirements" ADD CONSTRAINT "requirements_related_ticket_ticket_session_id_fk" FOREIGN KEY ("related_ticket") REFERENCES "tentix"."ticket_session"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "tentix"."requirements" ADD CONSTRAINT "requirements_related_ticket_ticket_sessions_id_fk" FOREIGN KEY ("related_ticket") REFERENCES "tentix"."ticket_session"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
