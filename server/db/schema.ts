...
export const contentBlockType = tentix.enum("content_block_type", [
	"text",
	"image",
	"code",
	"link",
...
export const ticketSession = tentix.table("ticket_session", {
	...
	description:
		json().$type<
			Array<{ type: "text" | "image" | "code" | "link"; content: string }>
		>(),
...
export const chatMessages = tentix.table(
	"chat_messages",
	{
		...
		senderId: integer("sender_id")
			.notNull()
			.references(() => users.id),
		attachment: uuid("attachment"), // main attachment id
		createdAt: timestamp("created_at", { precision: 6, mode: "string" })
			.defaultNow()
			.notNull(),
	},
...
export const messageContentBlocks = tentix.table(
	"message_content_blocks",
	{
		...
		position: smallint("position").notNull(), // Position in the message
		metadata: text("metadata"), // Metadata, such as code language, link preview, quote ID, etc.
	},
	(table) => [
		foreignKey({
			columns: [table.messageId],
			foreignColumns: [chatMessages.id],
			name: "message_content_blocks_message_id_chat_messages_id_fk",
		}),
	],
);
...
export const userSession = tentix.table(
	"user_session",
	{
		...
		cookie: char("cookie", { length: 128 }).primaryKey().notNull(),
	},
	(table) => [
		foreignKey({
			columns: [table.userId],
